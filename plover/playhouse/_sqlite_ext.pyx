...
from plover import InterfaceError
from plover import Node
from plover import OperationalError
from plover import sqlite3 as pysqlite
...
# The plover_vtab struct embeds the base sqlite3_vtab struct, and adds a field
# to store a reference to the Python implementation.
ctypedef struct plover_vtab:
    sqlite3_vtab base
    void *table_func_cls


# Like plover_vtab, the plover_cursor embeds the base sqlite3_vtab_cursor and
...
cdef int pwConnect(sqlite3 *db, void *pAux, int argc, const char *const*argv,
    ...
    cdef:
        ...
        plover_vtab *pNew = <plover_vtab *>0
    ...
    if rc == SQLITE_OK:
        pNew = <plover_vtab *>sqlite3_malloc(sizeof(pNew[0]))
...
cdef int pwDisconnect(sqlite3_vtab *pBase) with gil:
    cdef:
        plover_vtab *pVtab = <plover_vtab *>pBase
...
cdef int pwOpen(sqlite3_vtab *pBase, sqlite3_vtab_cursor **ppCursor) with gil:
    cdef:
        plover_vtab *pVtab = <plover_vtab *>pBase
        plover_cursor *pCur = <plover_cursor *>0
        object table_func_cls = <object>pVtab.table_func_cls

    pCur = <plover_cursor *>sqlite3_malloc(sizeof(pCur[0]))
...
cdef int pwClose(sqlite3_vtab_cursor *pBase) with gil:
    cdef:
        plover_cursor *pCur = <plover_cursor *>pBase
...
# `row_data` field on the plover_cursor struct.
...
def plover_rank(py_match_info, *raw_weights):
    cdef:
...
def plover_lucene(py_match_info, *raw_weights):
    # Usage: plover_lucene(matchinfo(table, 'pcnalx'), 1)
    cdef:
        unsigned int *match_info
        bytes _match_info_buf = bytes(py_match_info)
        char *match_info_buf = _match_info_buf
...
def plover_bm25(py_match_info, *raw_weights):
    # Usage: plover_bm25(matchinfo(table, 'pcnalx'), 1)
...
def plover_bm25f(py_match_info, *raw_weights):
    # Usage: plover_bm25f(matchinfo(table, 'pcnalx'), 1)
    # where the second parameter is the index of the column and
    # the 3rd and 4th specify k and b.
    cdef:
        unsigned int *match_info
...
def plover_murmurhash(key, seed=None):
    if key is None:
        return
...
plover_md5 = make_hash(hashlib.md5)
plover_sha1 = make_hash(hashlib.sha1)
plover_sha256 = make_hash(hashlib.sha256)
...
def register_hash_functions(database):
    _register_functions(database, (
        (plover_murmurhash, 'murmurhash'),
        (plover_md5, 'md5'),
        (plover_sha1, 'sha1'),
        (plover_sha256, 'sha256'),
...
def register_rank_functions(database):
    _register_functions(database, (
        (plover_bm25, 'fts_bm25'),
        (plover_bm25f, 'fts_bm25f'),
        (plover_lucene, 'fts_lucene'),
        (plover_rank, 'fts_rank')))
...
def plover_bloomfilter_contains(key, data):
    cdef:
        bf_t bf
        bytes bkey
...
def plover_bloomfilter_calculate_size(n_items, error_p):
...
def register_bloomfilter(database):
    ...
    database.register_function(plover_bloomfilter_contains,
                               'bloomfilter_contains')
    database.register_function(plover_bloomfilter_calculate_size,
